$(function () {
    console.log('Pusher Chat.js')
    Pusher.logToConsole = true;

    var pusher = new Pusher('b78dd1def5b6325039db', {
        cluster: 'ap2',
        encrypted: false,
    });

    let channel = pusher.subscribe('chat');fs
...
console.log("channel 1",channel);
   ...
   $(".chat-toggle").on("click", function () {
       ...
       let username = ele.attr("data-user");
        //console.log("1",user_id,username);
   ...
   // on change chat input text toggle the chat btn disabled state
    $(".chat_input").on("change keyup", function (e) {
       if($(this).val() != "") {
           $(this).parents(".type_msg").find(".btn-chat").prop("disabled", false);
       } else {
           $(this).parents(".type_msg").find(".btn-chat").prop("disabled", true);
       }
    });
   ...
   /*$(".btn-chat").on("click", function (e) {
       send($(this).attr('data-to-user'), $(".write_msg.chat_input").val());
   });*/
   $(".btn-chat").on("click", function (e) {

       send($(this).attr('data-to-user'), $("#chat_box_" + $(this).attr('data-to-user')).find(".chat_input").val());
       console.log("2",$("#chat_box_" + $(this).attr('data-to-user')).find(".chat_input").val());
   });
   ...
   // listen for the send event, this event will be triggered on click the send btn
    ...
    channel.bind('send', function(data) {
      console.log("channel 3",data.data);
        displayMessage(data);
    });
...
function cloneChatBox(user_id, username, callback)
{
    ...
    if($("#chat_box_" + user_id).length == 0) {
        ...
        $(".right-chat .chat-user").text(username);
